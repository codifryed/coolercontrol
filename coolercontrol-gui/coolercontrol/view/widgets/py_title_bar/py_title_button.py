...
from PySide6.QtGui import QPainter, QBrush, QColor, QPixmap, QMouseEvent, QPaintEvent
...
class PyTitleButton(QPushButton):
    ...
    def enterEvent(self, event: QMouseEvent) -> None:
        """Triggered when the mouse is over the button"""
    ...
    def icon_paint(self, qp: QPainter, image: str, rect: QRect) -> None:
        ...
        qp.drawPixmap(
            round((rect.width() - icon.width()) / 2),
            round((rect.height() - icon.height()) / 2),
            icon
        )
        painter.end()
