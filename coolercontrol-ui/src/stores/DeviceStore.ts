...
export const useDeviceStore =
    defineStore('device', () => {
        ...
        const currentDeviceStatus = ref(new Map<UID, Map<string, ChannelValues>>())
        ...
        function updateRecentDeviceStatus(): void {
            for (const [uid, device] of devices) {
                ...
                for (const temp of device.status.temps) {
                    currentDeviceStatus.value.get(uid)?.set(temp.frontend_name, {
                        temp: temp.temp.toFixed(1)
                    })
                }
                for (const channel of device.status.channels) { // This gives us both "load" and "speed" channels
                    currentDeviceStatus.value.get(uid)?.set(toTitleCase(channel.name), {
