...
const alert = computed(() => settingsStore.alerts.find((alert) => alert.uid === props.alertUID)!)
const currentName: string = alert.value.name
...
const closeAndSave = async (): Promise<void> => {
    ...
    alert.value.name = nameInput.value
    const successful = await settingsStore.updateAlert(alert.value.uid)
    if (successful) emit('nameChange', alert.value.name)
}
