...
const currentName: string = fun.value.name
...
const closeAndSave = async (): Promise<void> => {
    ...
    fun.value.name = nameInput.value
    const successful = await settingsStore.updateFunction(fun.value.uid)
    if (successful) {
        toast.add({
            severity: 'success',
            summary: t('common.success'),
            detail: t('views.functions.functionUpdated'),
            life: 3000,
        })
        ...
        toast.add({
            ...
            detail: t('views.functions.functionUpdateError'),
            life: 3000,
        })
    }
}
