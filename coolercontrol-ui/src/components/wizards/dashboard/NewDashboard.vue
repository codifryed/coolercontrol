...
const chosenName: Ref<string> = ref('New Dashboard')
...
const saveDashboard = async (): Promise<void> => {
    const newDashboard = new Dashboard(chosenName.value)
    settingsStore.dashboards.push(newDashboard)
    emitter.emit('dashboard-add-menu', { dashboardUID: newDashboard.uid })
