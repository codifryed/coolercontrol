...
const saveSensor = async () => {
  ...
  chosenTempSources.value.forEach((tempSource) =>
      tempSources.push(new CustomTempSourceData(
  ...
  <div class="grid">
    <div class="col-fixed" style="width: 22rem">
      ...
      <div class="p-float-label mt-5">
        <Dropdown v-model="selectedSensorType" inputId="dd-sensor-type" :options="sensorTypes"
                  placeholder="Type" class="w-full" scroll-height="400px"/>
        <label for="dd-sensor-type">Sensor Type</label>
      </div>
      <div class="p-float-label mt-5">
        <Dropdown v-model="selectedMixFunction" inputId="dd-mix-function" :options="mixFunctions"
                  placeholder="Type" class="w-full" scroll-height="400px"/>
        <label for="dd-mix-function">Mix Function</label>
      </div>
      <div class="p-float-label mt-4">
        <MultiSelect v-model="chosenTempSources" inputId="dd-temp-sources" :options="tempSources"
                     option-label="tempFrontendName" option-group-label="deviceName" option-group-children="temps"
                     placeholder="Temp Sources" :class="['w-full']" scroll-height="400px">
      ...
      <div v-if="selectedMixFunction === CustomSensorMixFunctionType.WeightedAvg" class="mt-4">
        <DataTable :value="chosenTempSources">
          <Column field="tempFrontendName" header="Temp Name">
            <template #body="slotProps">
              <span class="pi pi-minus mr-2" :style="{color: slotProps.data.lineColor}"/>{{ slotProps.data.tempFrontendName }}
            </template>
          </Column>
          <Column header="Weight">
            <template #body="slotProps">
              <InputNumber v-model="slotProps.data.weight" show-buttons :min="1" :max="254" :input-style="{width: '3.5rem'}"/>
            </template>
          </Column>
        </DataTable>
      </div>
