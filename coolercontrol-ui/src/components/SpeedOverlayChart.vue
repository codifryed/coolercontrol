...
const setGraphOffsetData = (profileIndex: number) => {
    ...
    const profile = memberProfiles.value[profileIndex]
    if (profile.p_type === ProfileType.Fixed) {
        if (props.profile.offset_profile != null && props.profile.offset_profile.length > 0) {
            const duty = profile.speed_fixed ?? 0
            const offsetDuty = calcDutyFromBaseProfileDuty(duty)
            graphOffsetLineData[profileIndex].push({ value: [axisXTempMin, offsetDuty] })
            graphOffsetLineData[profileIndex].push({ value: [axisXTempMax, offsetDuty] })
        }
        return
    }
    if (
