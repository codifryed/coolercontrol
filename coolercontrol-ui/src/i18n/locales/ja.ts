    ...
    layout: {
        ...
        settings: {
            ...
            tooltips: {
                ...
                daemonSsl:
                    'SSL/TLSを使用してデーモンに接続するかどうか。プロキシセットアップが必要です。',
                applyOnBoot: 'デーモン起動時およびスリープから復帰時に自動的に設定を適用します',
                startupDelay:
                    'デバイス通信を開始する前の遅延（秒単位）。初期化に時間がかかるデバイスや断続的に検出されるデバイスに役立ちます',
                thinkPadFanControl:
                    'これはThinkPad ACPIファン制御を有効にするためのヘルパーです。安全上の理由から、ファン制御操作はデフォルトで無効になっています。CoolerControlはこれを有効にしようとしますが、ハードウェアへのリスクを認識する必要があります。自己責任で進めてください。',
                thinkPadFullSpeed:
                    'Thinkpadラップトップの場合、これは全速モードを有効にします。これにより、100%に設定されたときにファンが絶対最大まで回転できるようになりますが、ファンが仕様を超えて動作し、摩耗が増加します。注意して使用してください。',
                compress:
                    'APIペイロードサイズを減らすために応答圧縮を有効にしますが、CPU使用率が増加することに注意してください。',
                liquidctlNoInit:
                    '注意：あなた自身または別のプログラムがliquidctlデバイスの初期化を処理している場合にのみ、これを無効にしてください。これは他のプログラムとの競合を避けるのに役立ちます。',
                hideDuplicate:
                    '一部のデバイスはLiquidctlとHWMonの両方のドライバーでサポートされています。Liquidctlはその追加機能のためにデフォルトで使用されます。代わりにHWMonドライバーを使用するには、これとliquidctlデバイスを無効にして、ドライバーの競合を避けてください。',
                pollRate:
                    'センサーデータがポーリングされるレート（秒単位）。\nより高いポーリングレートはリソース使用量を減らし、より低いレートは応答性を高めます。\n1.0未満のレートは注意して使用してください。',
