...
const noneLightingMode = new LightingMode(
    'none',
    'None',
    0,
    0,
...
let startingSpeed: string = lightingSpeeds.length === 0
    ? 'none'
...
if (startingDeviceSetting?.lighting != null) {
  startingMode = lightingModes.find((mode: LightingMode) => mode.name === startingDeviceSetting.lighting?.mode) ?? noneLightingMode
  if (startingMode.speed_enabled) {
    startingSpeed = lightingSpeeds.find((speed: string) => speed === startingDeviceSetting.lighting?.speed) ?? startingSpeed
  }
  ...
  if (startingMode.max_colors > 0) {
    startingNumberOfColors = startingDeviceSetting.lighting.colors.length ?? startingNumberOfColors
...
const setNewColor = (colorIndex: number, newColor: string | null) => {
  if (newColor == null) {
    if (startingDeviceSetting?.lighting != null && startingDeviceSetting.lighting.colors.length > colorIndex) {
    ...
    <div class="grid">
      <div class="col-fixed" style="width: 16rem">
        ...
        <div v-if="selectedMode.backward_enabled" class="mt-5">
          ...
          <ToggleButton v-model="selectedBackwardEnabled" on-label="Backward" off-label="Forward" class="w-full"/>
        </div>
        <div v-if="selectedMode.max_colors > 0" class="p-float-label mt-6">
          <InputNumber v-model="selectedNumberOfColors" showButtons buttonLayout="horizontal" class="w-full"
                       :min="selectedMode.min_colors" :max="selectedMode.max_colors" :input-style="{width: '58px'}"
