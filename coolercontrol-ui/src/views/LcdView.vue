...
if (startingDeviceSetting?.lcd != null) {
  ...
  const savedTempSource: TempSource | undefined = startingDeviceSetting.lcd.temp_source != null
  ...
  if (savedTempSource != null) {
    outer: for (const tempDevice of tempSources.value) {
      for (const tempSource of tempDevice.temps) {
        if (tempSource.deviceUID === savedTempSource.device_uid
...
const validateFileSize = (file: File): void => {
  ...
  toast.add({severity: 'error', summary: 'Error', detail: 'Image is too large. Please choose a smaller one.', life: 4000})
}
...
const validateFileType = (file: File): void => {
  ...
  toast.add({severity: 'error', summary: 'Error', detail: 'Image does not register as an image type', life: 4000})
}
...
const saveLCDSetting = async () => {
  ...
  if (setting.mode === 'image' && files.length > 0) {
    await settingsStore.saveDaemonDeviceSettingLcdImages(props.deviceId, props.name, setting, files)
  } else {
...
watch(fileDataURLs.value, () => {
  ...
  const img: HTMLImageElement = document.getElementById("lcd-image") as HTMLImageElement ?? new HTMLImageElement()
    ...
    <div class="grid">
      <div class="col-fixed" style="width: 16rem">
        ...
        <div v-if="selectedLcdMode.brightness" class="p-float-label mt-6">
          <InputNumber placeholder="Brightness" v-model="selectedBrightness" inputId="dd-brightness" mode="decimal"
                       class="w-full" suffix="%" :step="1" :input-style="{width: '60px'}" :min="0" :max="100"/>
          <Slider v-model="selectedBrightness" :step="1" :min="0" :max="100" class="w-full mt-0"/>
          <label for="dd-brightness">Brightness</label>
        </div>
        <div v-if="selectedLcdMode.orientation" class="p-float-label mt-6">
          <InputNumber placeholder="Orientation" v-model="selectedOrientation" inputId="dd-orientation" mode="decimal"
                       class="w-full" suffix="°" :step="90" :input-style="{width: '60px'}" :min="0" :max="270"
                       readonly/>
          <Slider v-model="selectedOrientation" :step="90" :min="0" :max="270" class="w-full mt-0"/>
          <label for="dd-brightness">Orientation</label>
        </div>
        <div v-if="selectedLcdMode.type === LcdModeType.CUSTOM && selectedLcdMode.name === 'temp'"
