...
fn get_most_recent_status(device: RwLockReadGuard<Device>, smoothing_level: u8) -> DeviceStatusDto {
    let sample_size = if smoothing_level == 0 {
        1
    } else {
        (smoothing_level * utils::SMA_WINDOW_SIZE) as usize
    };
...
fn smooth_all_temps_and_loads(device_dto: &mut DeviceStatusDto, smoothing_level: u8) {
    ...
    device_dto
        ...
        .for_each(|temp_status| {
            all_temps
                .entry(temp_status.name.to_owned())
                .or_insert_with(Vec::new)
                .push(temp_status.temp.to_owned())
        });
    let all_loads: Vec<f64> = device_dto
        .status_history
        .iter()
        .filter_map(|d_status| {
            d_status
                .channels
                .iter()
                .find(|channel| channel.name.to_lowercase().contains("load"))
        })
    ...
    let smoothed_loads =
        utils::all_values_from_simple_moving_average(all_loads.as_slice(), smoothing_level);
    for (i, d_status) in device_dto.status_history.iter_mut().enumerate() {
        d_status.temps.iter_mut().for_each(|temp_status| {
