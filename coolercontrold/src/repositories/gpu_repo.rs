...
impl GpuRepo {
    ...
    async fn request_nvml_status(&self, nv_info: Arc<NvidiaDeviceInfo>) -> StatusNvidiaDeviceNvml {
    ...
    async fn load_nvml_status(self: Arc<Self>, tasks: &mut Vec<JoinHandle<()>>) {
        for (uid, nv_info) in &self.nvidia_device_infos {
            if let Some(device_lock) = self.devices.get(uid) {
                ...
                let join_handle = tokio::task::spawn(async move {
                    let nvml_status = self_ref.request_nvml_status(nv_info).await;
                    self_ref.nvidia_preloaded_statuses.write().await.insert(
                        type_index,
