...
logging.basicConfig(
    ...
    format=log_format,
    handlers=[
        logging.FileHandler("rdna3_test.log"),
...
class RDNA3Test:
    ...
    def get_fan_curve(self) -> list[(int, int)]:
        ...
        for line in self.fan_curve_path.read_text().splitlines():
            match = re.search(r'(?P<index>\d+):\s+(?P<temp>\d+)C\s+(?P<duty>\d+)%', line)
            if match is None:
                continue
    ...
    def determine_fan_curve_limits(self):
        for line in self.fan_curve_path.read_text().splitlines():
            ...
            if duty_match is not None:
                ...
                continue
        if self.temp_min < 0 or self.temp_max < 0 or self.duty_min < 0 or self.duty_max < 0:
            log.error(
    ...
    def reset_fan_curve(self):
        if self.args.test:
            log.info(f"TEST Resetting fan curve")
            return
        try:
            log.info(f"Resetting fan curve")
    ...
    def commit_fan_curve_changes(self):
        if self.args.test:
            log.info(f"TEST Committing new fan curve")
            return
        try:
    ...
    def apply_flat_fan_curve(self, duty: int) -> None:
        if duty < self.duty_min or duty > self.duty_max:
            log.error(
                f"Invalid duty: {duty}. "
                f"Must be between allowed limits of {self.duty_min} and {self.duty_max}"
        ...
        new_fan_curve = []
        steps = [
            float(self.temp_min) + x * (self.temp_max - self.temp_min) / float(
                self.fan_curve_size - 1)
    ...
    def apply_linear_fan_curve(self, starting_duty: int, ending_duty: int) -> None:
        if starting_duty < self.duty_min or starting_duty > self.duty_max:
            log.error(
                f"Invalid starting duty: {starting_duty}. "
                f"Must be between allowed limits of {self.duty_min} and {self.duty_max}"
            )
            return
        if ending_duty < self.duty_min or ending_duty > self.duty_max:
            log.error(
                f"Invalid ending duty: {ending_duty}. "
                f"Must be between allowed limits of {self.duty_min} and {self.duty_max}"
            )
            return
        new_fan_curve = []
        ...
        for temp in steps:
            duty = starting_duty + (ending_duty - starting_duty) * (temp - self.temp_min) // (
                self.temp_max - self.temp_min)
            new_fan_curve.append((int(temp), int(duty)))
        self.set_fan_curve(new_fan_curve)


def wait_for_fan_stabilization(seconds: int) -> None:
...
def main():
    ...
    log.info("Applying flat 50% fan curve")
    log.info("##################################################")
    test.apply_flat_fan_curve(50)
    wait_for_fan_stabilization(3)
    test.read_sensors()

    log.info("Applying flat 20% fan curve")
    log.info("##################################################")
    test.apply_flat_fan_curve(20)
    wait_for_fan_stabilization(3)
    test.read_sensors()

    log.info("Applying flat 80% fan curve")
    log.info("##################################################")
    test.apply_flat_fan_curve(80)
    wait_for_fan_stabilization(3)
    test.read_sensors()
    ...
    wait_for_fan_stabilization(5)
    test.read_sensors()

    log.info("Steady rapid changes test")
    log.info("##################################################")
    wait_for_fan_stabilization(1)
    duty_changes = [30, 50, 70, 50, 30, 50, 70, 50, 30, 50, 70, 50, 30]

    start_time = time()
    for duty in duty_changes:
        test.apply_flat_fan_curve(duty)
        max_1_sec_wait(start_time)
        start_time = time()
        test.read_sensors()
    wait_for_fan_stabilization(5)
    ...
    wait_for_fan_stabilization(wait_seconds)
    test.read_sensors()
    test.apply_flat_fan_curve(50)
    wait_for_fan_stabilization(wait_seconds)
    test.read_sensors()
